<?php

//start the session
session_start();

//Database connection
include ('cn/cn.php');

$Uname = mysqli_real_escape_string($dbc, $_SESSION['Username']);
$BranchID = mysqli_real_escape_string($dbc, $_SESSION['BranchID']);
$ActiveDate= mysqli_real_escape_string($dbc, $_SESSION['ActiveDay']);
$e= mysqli_real_escape_string($dbc,$_POST['e']);

if(!isset($_SESSION['Uname'])){
	
	header('Location: login');
	
}elseif($e==''){
    die('');
}elseif(!isset ($e)){
    die('Search key not found'); 
}else{
    $result = mysqli_query($dbc,"select * from consignee_main where FullName like '%$e%' or TelNo like '%$e%'");
    
  while( $f = mysqli_fetch_assoc($result)){
       //echo '<div class="student_search_display-wrap"><span class="search_student_details first-search-span">'.$f['StudentID'].'</span><span class="search_student_details second-search-span">'.$f['FullName'].'</span><span class="search_student_details third-search-span">'.$f['CurrentClass'].'</span>'
         //      . '<div class="wrap-details"></div></div><br>';
      
      echo "<div class='return_search_results consignee_details_0_search hide_div_note' cns='$f[ConsigneeID]'>".$f['FullName']."  <b style='color:red;'>| ".$f['TelNo']." </b> </div>";
  }
   
  
}


?>

<script>
    $('.consignee_details_0_search').click(function(){
        
       var cns = $(this).attr('cns');
       var m = $(this).text();
        
       //$('#edit_std_info_display_board').html(e);
       $('#client_nonm_bill_view').val(m);
       $('#client_id_nom_bill_search').text(cns);
       //$('#hBL_conisgnee_weight').focus();
       
       $('#view_other_report_search_result').html('');
		$.get(
			'load_client_invoice_report.php',
			{ cid:cns },
			function (a) {
				$('#view_other_report_search_result').html(a);
				$('.progress-loader').remove();
			});

        /* $.post('insert_multi_values_0.php',{e3:cns},function(){
           $('#view_other_report_search_result').load('load_client_invoice_report.php');
        })
 */
       //$('#sel_ots_acc_invoice').load('load_sel_billing_account.php');
       //$('#sel_ots_acc_invoice_nonm').load('load_sel_billing_account.php');
       //$('#client_desc_invoice_nonm').val('');
       // $('#sel_bl_invoice_nonm').focus();
       $('.consignee_details_0_search').toggle();
    });
</script>
