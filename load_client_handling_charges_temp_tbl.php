...
if(!isset( $_SESSION['Uname'])){	
   ...
   $a = mysqli_query($dbc, "select * from temp_other_invoice_view_0 where Username='$Uname' order by AccountName");
...
<script>
    ...
    $('.client_tax_charge').click(function(){
       ...
       if($(this).prop("checked")==true){
           $.post('add_client_tax.php',{acc:acc},function(a){
               if(a==1){
           ...
           $.post('remove_client_tax.php',{acc:acc},function(a){
              if(a==1){
                  $('#client_charges_display_details').load('load_client_handling_charges_temp_tbl.php'); 
               }else{
                   alert(a);
  ...
  $('#btn_save_client_invoice').click(function(){
    ...
    if(q){
        $.post('add_new_client_charges.php',{rid,rno,cid,dot,name,desc},function(a){
            if(a==1){
                $('.ep').text('');
                $('.ep').val('');
                $('#client_charges_display_details').load('load_client_handling_charges_temp_tbl.php');
 ...
 $('.rm-client-charges').click(function(){
    ...
    if(q){
        ...
        $.post('remove_client_charge_account_temp.php',{cid:acc},function(a){
            if(a==1){
